SELECT
    CAB.CODEMP
     ...
     , CASE
            WHEN CAB.TIPMOV = 'V' THEN (SELECT CUS.CUSSEMICM * ITE.QTDNEG FROM TGFCUS CUS WHERE CUS.CODEMP = CAB.CODEMP AND CUS.CODPROD = ITE.CODPROD AND DTATUAL = (SELECT MAX(C.DTATUAL) FROM TGFCUS C WHERE C.CODEMP = CUS.CODEMP AND C.CODPROD = CUS.CODPROD AND C.DTATUAL <= CAB.DTNEG)) * (1)
            WHEN CAB.TIPMOV = 'D' THEN (SELECT CUS.CUSSEMICM * ITE.QTDNEG FROM TGFCUS CUS WHERE CUS.CODEMP = CAB.CODEMP AND CUS.CODPROD = ITE.CODPROD AND DTATUAL = (SELECT MAX(C.DTATUAL) FROM TGFCUS C WHERE C.CODEMP = CUS.CODEMP AND C.CODPROD = CUS.CODPROD AND C.DTATUAL <= CAB.DTNEG)) * (-1)
      ELSE 0 END AS CUSTO
