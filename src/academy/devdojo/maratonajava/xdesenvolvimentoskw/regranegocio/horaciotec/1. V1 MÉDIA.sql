SELECT
		...
		, (SELECT SUM(I.VLRSUBST) FROM TGFCAB C INNER JOIN TGFITE I ON C.NUNOTA = I.NUNOTA WHERE I.CODPROD = ITE.CODPROD AND I.CODCFO IN ('2401', '2403')
					AND C.TIPMOV IN ('C') AND C.STATUSNOTA = 'L' AND C.CODEMP = CAB.CODEMP AND I.CODTRIB IN (10,30,60,70) AND C.DTENTSAI <= CAB.DTNEG AND CAB.CODTIPOPER NOT IN (213,215,216,217,219)) AS VALOR_ST_DA_COMPRA

		, (SELECT CASE WHEN NVL(SUM(I.VLRSUBST),0) = 0 THEN 0 ELSE SUM(I.VLRSUBST)/SUM(I.QTDNEG) END FROM TGFCAB C INNER JOIN TGFITE I ON C.NUNOTA = I.NUNOTA WHERE I.CODPROD = ITE.CODPROD AND I.CODCFO IN ('2401', '2403')
                    AND C.TIPMOV IN ('C') AND C.STATUSNOTA = 'L' AND C.CODEMP = CAB.CODEMP AND I.CODTRIB IN (10,30,60,70) AND C.DTENTSAI <= CAB.DTNEG AND CAB.CODTIPOPER NOT IN (213,215,216,217,219))*ITE.QTDNEG AS ST_MEDIO_DA_COMPRA
    ...
    AND CAB.STATUSNFE ='A'
    AND ITE.SEQUENCIA > 0
    AND ITE.CODCFO IN (5927, 6101, 6102, 6107, 6108, 6116, 6117, 6202, 6403, 6404, 6411, 6655, 6656, 6927, 6949)
    AND ITE.CODPROD IN (SELECT I.CODPROD FROM TGFCAB C INNER JOIN TGFITE I ON C.NUNOTA = I.NUNOTA WHERE I.CODPROD = ITE.CODPROD AND I.CODCFO IN ('2401', '2403')
                        AND C.TIPMOV IN ('C') AND C.STATUSNOTA = 'L' AND C.CODEMP = CAB.CODEMP AND I.CODTRIB IN (10,30,60,70) AND C.DTENTSAI <= CAB.DTNEG AND CAB.CODTIPOPER NOT IN (213,215,216,217,219))