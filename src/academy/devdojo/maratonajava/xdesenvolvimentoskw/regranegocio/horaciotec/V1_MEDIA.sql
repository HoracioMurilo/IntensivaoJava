...
-- ORDER BY NVL(Z.HRENTSAI,Z.DTENTSAI) ASC
     ...
     DATAINI AS (
         SELECT
             MIN(NVL(HRENTSAI,DTENTSAI)) AS START_DATE
         FROM DADOS
...
SELECT SUM(VLRTOT) / SUM(SALDO) AS MEDIA_PONDERADA FROM DADOS WHERE SALDO > 0 AND NVL(HRENTSAI,DTENTSAI) <= (SELECT START_DATE FROM DATAINI) AND CODEMP > 0